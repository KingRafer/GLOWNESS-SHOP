...
let state = {
  ...
  formModal:null,
  products:[], orders:[], users:[], withdrawals:[], balanceLogs:[], settings:SEED_SETTINGS,
...
function toast(msg){
  ...
  /* Saat form checkout/alamat terbuka, jangan render ulang (isian form akan hilang) */
  const keepForm = state.formModal && (state.formModal.type==='checkout' || state.formModal.type==='address');
  if(keepForm){
    const old = document.getElementById('floating-toast'); if(old) old.remove();
    const el = document.createElement('div');
    el.className = 'toast'; el.id = 'floating-toast'; el.style.zIndex = 9999;
    el.innerHTML = '<span class="dotg"></span>' + esc(msg);
    document.body.appendChild(el);
    window.__t = setTimeout(()=>{ const x=document.getElementById('floating-toast'); if(x) x.remove(); },2600);
    return;
  }
  state.toast = msg; render();
  window.__t = setTimeout(()=>{state.toast=null; render();},2600);
}
...
/* =====================================================
   ALAMAT PENGIRIMAN
   - Buku alamat disimpan di user: u.addresses[], u.defaultAddressId
   - Setiap pesanan menyimpan salinan alamat di order.shipping
===================================================== */
function userAddresses(u){ return (u && u.addresses) || []; }
function fmtAddress(a){
  if(!a) return '';
  return [a.street, a.district, a.city, a.province].filter(Boolean).join(', ') + (a.postal ? ' ' + a.postal : '');
}
function addrFields(p, a){
  a = a || {};
  const f = (id,label,val,ph,extra)=>`<div class="field"><label>${label}</label><input id="${p}-${id}" value="${esc(val||'')}" placeholder="${ph||''}" ${extra||''}></div>`;
  const grid = 'display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:0 12px;';
  return `
  <div class="field"><label>Label Alamat</label><select id="${p}-label">${['Rumah','Kantor','Kos/Apartemen','Lainnya'].map(l=>`<option ${a.label===l?'selected':''}>${l}</option>`).join('')}</select></div>
  <div style="${grid}">
    ${f('recipient','Nama Penerima',a.recipient,'Nama penerima')}
    ${f('phone','No. HP Penerima',a.phone,'08xxxxxxxxxx','inputmode="tel"')}
  </div>
  ...
  <div style="${grid}">
    ${f('district','Kelurahan / Kecamatan',a.district,'Contoh: Medan Petisah')}
    ${f('city','Kota / Kabupaten',a.city,'Contoh: Medan')}
  </div>
  <div style="${grid}">
    ${f('province','Provinsi',a.province,'Contoh: Sumatera Utara')}
    ${f('postal','Kode Pos',a.postal,'5 digit','inputmode="numeric" maxlength="5"')}
  </div>
  ${f('note','Patokan / Catatan Kurir (opsional)',a.note,'Contoh: pagar hitam, dekat masjid')}`;
}
function readAddrFields(p){
  const g = id => { const el = document.getElementById(p+'-'+id); return el ? el.value.trim() : ''; };
  return {label:g('label')||'Rumah', recipient:g('recipient'), phone:g('phone'), street:g('street'), district:g('district'), city:g('city'), province:g('province'), postal:g('postal'), note:g('note')};
}
function validateAddress(a){
  if(!a.recipient) return 'Nama penerima wajib diisi';
  const ph = (a.phone||'').replace(/\D/g,'');
  if(ph.length<9 || ph.length>15) return 'No. HP penerima tidak valid';
  if(!a.street || a.street.length<8) return 'Alamat lengkap wajib diisi (jalan, nomor, RT/RW)';
  if(!a.district) return 'Kelurahan/Kecamatan wajib diisi';
  if(!a.city) return 'Kota/Kabupaten wajib diisi';
  if(!a.province) return 'Provinsi wajib diisi';
  if(!/^\d{5}$/.test(a.postal||'')) return 'Kode pos harus 5 digit';
  return null;
}
function addressBlockHtml(a){
  if(!a) return '';
  return `<div style="font-size:13px;line-height:1.6;"><b>${esc(a.recipient)}</b> · ${esc(a.phone)}<br>${esc(fmtAddress(a))}${a.note?`<br><span style="color:var(--text-dimmer);">Catatan: ${esc(a.note)}</span>`:''}</div>`;
}
function addressShortHtml(o){
  if(!o || !o.shipping) return '';
  return `<div class="field-hint" style="margin-top:4px;max-width:260px;">📍 ${esc(o.shipping.recipient)} · ${esc(fmtAddress(o.shipping))}</div>`;
}
function toggleNewAddr(){
  const sel = document.querySelector('input[name="co-addr"]:checked');
  const box = document.getElementById('co-addr-new');
  if(box) box.style.display = (!sel || sel.value==='__new') ? 'block' : 'none';
}
/* Ambil alamat dari form checkout. Return null (+toast) bila belum lengkap. */
function readCheckoutAddress(name, wa){
  const u = currentUser();
  const sel = document.querySelector('input[name="co-addr"]:checked');
  if(sel && sel.value!=='__new'){
    const a = userAddresses(u).find(x=>x.id===sel.value);
    if(a) return {...a};
  }
  const a = readAddrFields('ca');
  if(!a.recipient) a.recipient = name;
  if(!a.phone) a.phone = wa;
  const err = validateAddress(a);
  if(err){ toast(err); return null; }
  a._new = true;
  a._save = !!(u && (document.getElementById('ca-save')||{}).checked);
  return a;
}
/* Simpan ke buku alamat (bila dipilih) & kembalikan salinan bersih untuk order */
function commitCheckoutAddress(a){
  const u = currentUser();
  const ship = {...a}; delete ship._new; delete ship._save;
  if(a._new && a._save && u){
    if(!u.addresses) u.addresses = [];
    ship.id = 'a'+Date.now();
    u.addresses.push({...ship});
    if(!u.defaultAddressId) u.defaultAddressId = ship.id;
  }
  return ship;
}
...
===================================================== */
function buildWaMessage({name, wa, items, subtotal, total, refCode, orderId, paymentName, shipping}){
  let lines = [];
  ...
  if(shipping){
    ...
    lines.push(`${shipping.recipient} (${shipping.phone})`);
    lines.push(fmtAddress(shipping));
    if(shipping.note) lines.push(`Catatan: ${shipping.note}`);
  }
  lines.push('');
...
function CheckoutModal(){
  ...
  const addrs = userAddresses(u);
  const defId = (addrs.find(a=>a.id===(u&&u.defaultAddressId)) || addrs[0] || {}).id;
  ...
  <div class="overlay" onclick="if(event.target===this) closeFormModal()">
    <div class="modal">
      ...
      <form onsubmit="submitCheckout(event)">
        ...
        <div class="field"><label>Alamat Pengiriman <span style="color:var(--red);">*</span></label>
          ${addrs.length?`<div class="pay-list" style="grid-template-columns:1fr;">${addrs.map(a=>`
            <label class="pay-opt"><input type="radio" name="co-addr" value="${a.id}" ${a.id===defId?'checked':''} onchange="toggleNewAddr()">
              <span class="pay-ic">📍</span><span><b>${esc(a.label)} · ${esc(a.recipient)}</b><small>${esc(fmtAddress(a))}</small></span></label>`).join('')}
            <label class="pay-opt"><input type="radio" name="co-addr" value="__new" onchange="toggleNewAddr()"><span class="pay-ic">➕</span><span><b>Pakai alamat baru</b></span></label></div>`:`<div class="field-hint" style="margin:0 0 10px;">Isi alamat lengkap agar pesanan bisa langsung diantar.</div>`}
          <div id="co-addr-new" style="display:${addrs.length?'none':'block'};margin-top:12px;">
            ${addrFields('ca', {recipient:''})}
            <div class="field-hint" style="margin:-8px 0 10px;">Nama & HP penerima boleh dikosongkan bila sama dengan data pembeli di atas.</div>
            ${u?`<label style="display:flex;align-items:center;gap:8px;font-size:13px;"><input type="checkbox" id="ca-save" checked style="width:auto;">Simpan ke buku alamat</label>`:''}
          </div>
        </div>
...
function buildOrderObject(name, wa, refInput, paymentMethod, shipping){
  ...
  const order = {
    ...
    items, paymentMethod: paymentMethod||'whatsapp', shipping: shipping||null,
...
function submitCheckout(e){
  ...
  const addr = readCheckoutAddress(name, wa);
  if(!addr) return;
  const shipping = commitCheckoutAddress(addr);
  const u = currentUser();
  const {order, c} = buildOrderObject(name, wa, refInput, pm ? pm.name : 'Konfirmasi WhatsApp', shipping);
...
function PaymentInfoModal(){
  ...
  <div class="overlay" onclick="if(event.target===this) closeFormModal()">
    <div class="modal">
      ...
      ${o.shipping?`<div style="padding:12px 16px;background:var(--card);border-radius:var(--radius-md);margin-bottom:16px;"><div style="font-size:12px;color:var(--text-dimmer);margin-bottom:4px;">📍 Dikirim ke</div>${addressBlockHtml(o.shipping)}</div>`:''}
...
function confirmPaymentWa(id){
  ...
  openWhatsAppOrder({
    orderId:o.id, name:o.buyerName, wa:o.buyerWa, items:o.items,
    subtotal:o.items.reduce((a,i)=>a+i.unitPrice*i.qty,0), total:o.total,
    refCode:o.refCode||null, paymentName:o.paymentMethod, shipping:o.shipping||null,
  });
}
async function payWithMidtrans(){
  ...
  if(!addr) return;
  const shipping = commitCheckoutAddress(addr);
  const {order} = buildOrderObject(name, wa, refInput, 'midtrans', shipping);
  ...
  try{
    const res = await fetch(`${FUNCTIONS_BASE}/midtrans-snap`, {
      ...
      body: JSON.stringify({
        ...
        shipping_address: { first_name: shipping.recipient, phone: shipping.phone, address: shipping.street, city: shipping.city, postal_code: shipping.postal, country_code: 'IDN' },
...
function pageAkunAlamat(u){
  ...
  return `
  <div class="pg-head"><div><h1>Alamat Saya</h1><p>Alamat tujuan pengiriman pesananmu. Wajib ada alamat saat memesan.</p></div>
    <button class="btn btn-primary" onclick="openAddressForm()">+ Tambah Alamat</button></div>
  ${list.length===0?`<div class="panel"><div class="empty-state"><div class="em">📍</div>Belum ada alamat tersimpan.</div></div>`:
  list.map(a=>{
    const isDef = a.id===u.defaultAddressId;
    return `<div class="panel">
      <div class="panel-head"><h3>${esc(a.label)} ${isDef?'<span class="badge badge-green" style="margin-left:6px;">Utama</span>':''}</h3>
        <div class="row-actions">
          ${isDef?'':`<button class="btn btn-soft btn-sm" onclick="setDefaultAddress('${a.id}')">Jadikan Utama</button>`}
          <button class="icon-btn" title="Edit" onclick="openAddressForm('${a.id}')">✎</button>
          <button class="icon-btn danger" title="Hapus" onclick="deleteAddress('${a.id}')">🗑</button>
        </div></div>
      ${addressBlockHtml(a)}
    </div>`;}).join('')}`;
}
...
function AddressFormModal(){
  ...
  const isDef = a ? a.id===u.defaultAddressId : userAddresses(u).length===0;
  return `<div class="overlay" onclick="if(event.target===this) closeFormModal()"><div class="modal">
    <div class="modal-top"><h3>${a?'Edit':'Tambah'} Alamat</h3><button class="x-btn" onclick="closeFormModal()">✕</button></div>
    <form onsubmit="submitAddressForm(event)">
      ${addrFields('af', a || {recipient:u.name})}
      <label style="display:flex;align-items:center;gap:8px;font-size:13px;margin-bottom:16px;"><input type="checkbox" id="af-default" ${isDef?'checked':''} style="width:auto;">Jadikan alamat utama</label>
...
function submitAddressForm(e){
  ...
  const data = readAddrFields('af');
  const err = validateAddress(data);
  if(err){ toast(err); return; }
  if(!u.addresses) u.addresses = [];
  const id = state.formModal.id;
  let addr;
  if(id){ addr = u.addresses.find(x=>x.id===id); Object.assign(addr, data); }
  else { addr = {id:'a'+Date.now(), ...data}; u.addresses.push(addr); }
  if(document.getElementById('af-default').checked || !u.defaultAddressId) u.defaultAddressId = addr.id;
  persist(); state.formModal=null; render(); toast('Alamat disimpan');
}
function setDefaultAddress(id){ const u=currentUser(); u.defaultAddressId=id; persist(); render(); toast('Alamat utama diubah'); }
function deleteAddress(id){
  if(!confirm('Hapus alamat ini?')) return;
  const u = currentUser();
  u.addresses = userAddresses(u).filter(a=>a.id!==id);
  if(u.defaultAddressId===id) u.defaultAddressId = u.addresses[0] ? u.addresses[0].id : null;
...
function renderAkunCustomer(){
  ...
  return `<div class="dash-wrap">${CustomerSidebar(u, state.route)}<div class="dash-main">${content}</div></div>${PaymentInfoModal()}${AddressFormModal()}${Toast()}`;
}
function ordersTableSimple(orders, payActions){
  ...
  return `<div class="table-scroll"><table><thead><tr><th>ID</th><th>Produk</th><th>Total</th><th>Status</th><th>Pembayaran</th><th></th></tr></thead><tbody>
    ${orders.map(o=>{
      ...
      return `<tr><td>#${o.id}</td><td>${o.items.map(i=>i.name+' x'+i.qty).join(', ')}${addressShortHtml(o)}</td><td>${fmtRp(o.total)}</td><td>${statusBadge(o.status)}</td><td>${payStatusBadge(o)}${st==='Ditolak'&&o.paymentNote?`<div class="field-hint" style="margin-top:4px;">${esc(o.paymentNote)}</div>`:''}</td><td>${act}</td></tr>`;
    }).join('')}
  </tbody></table></div>`;
}
...
/* ---------- KOMISI PENDING & SALDO MANUAL ----------
   ...
   dicatat di state.balanceLogs. Saldo tersedia = total isian admin − penarikan (Menunggu/Disetujui). */
...
function sellerCredited(id){ return (state.balanceLogs||[]).filter(l=>l.sellerId===id).reduce((a,l)=>a+Number(l.amount||0),0); }
function sellerReserved(id){ return state.withdrawals.filter(w=>w.sellerId===id && w.status!=='Ditolak').reduce((a,w)=>a+w.amount,0); }
function sellerBalance(id){ return sellerCredited(id) - sellerReserved(id); }
function sellerPending(u){ return Math.max(sellerCommissionTotal(u) - sellerCredited(u.id), 0); }
  ...
  <div class="dash-wrap">
    ...
    <div class="dash-main">
      ...
      <div class="panel" style="margin-bottom:22px;">
        <div class="panel-head"><h3>Riwayat Pengisian Saldo</h3></div>
        <div class="table-scroll"><table><thead><tr><th>Tanggal</th><th>Jumlah</th><th>Catatan</th></tr></thead><tbody>
        ${logs.length===0?`<tr><td colspan="3"><div class="empty-state"><div class="em">🧾</div>Belum ada saldo yang diisi admin.</div></td></tr>`:
...
function pageStaffDashboard(){
  ...
  <div class="stat-grid">
    ...
    <div class="stat-card"><div class="lbl">Komisi Mitra</div><div class="val">${fmtRp(commission)}</div><div class="sub">${pendingWithdraw} penarikan menunggu</div></div>
...
function ordersTableStaff(orders){
  ...
  return `<div class="table-scroll"><table><thead><tr><th>ID</th><th>Pembeli</th><th>Produk</th><th>Total</th><th>Pembayaran</th><th>Bukti Bayar</th><th>Referral</th><th>Status</th><th></th></tr></thead><tbody>
    ${orders.map(o=>`<tr>
      <td>#${o.id}</td><td>${o.buyerName}${o.buyerWa?`<div class="field-hint">${esc(o.buyerWa)}</div>`:''}${o.shipping?`<div style="font-size:12px;margin-top:6px;max-width:280px;line-height:1.5;">📍 <b>${esc(o.shipping.recipient)}</b> · ${esc(o.shipping.phone)}<br>${esc(fmtAddress(o.shipping))}${o.shipping.note?`<br><i>${esc(o.shipping.note)}</i>`:''}</div>`:'<div class="field-hint">Alamat: —</div>'}</td><td>${o.items.map(i=>i.name+' x'+i.qty).join(', ')}</td><td>${fmtRp(o.total)}</td>
...
function pageStaffPengguna(){
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Info</th><th></th></tr></thead><tbody>
    ${state.users.map(u=>`<tr>
      ...
      <td>${u.role==='member'?'Kode: '+u.refCode+' · '+fmtRp(u.totalSpend||0)+' belanja':'—'}</td>
...
function pageStaffSeller(){
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Mitra</th><th>Kode Referral</th><th>Upline</th><th>Jaringan</th><th>Order</th><th>Komisi Pending</th><th>Saldo</th><th></th></tr></thead><tbody>
    ${sellers.map(s=>{
      ...
      return `<tr><td><b>${s.name}</b></td>
        ...
        <td>${(state.users.find(x=>x.id===s.parentId)||{}).name||'—'}</td><td>${downlineCount(s.id)}</td><td>${os.length}</td><td>${fmtRp(pend)}</td><td><b>${fmtRp(bal)}</b></td>
        <td class="row-actions"><button class="btn btn-soft btn-sm" onclick="openBalanceForm('${s.id}')" title="Isi saldo mitra">+ Isi Saldo</button><button class="icon-btn" onclick="openUserForm('${s.id}')" title="Edit lengkap">✎</button></td></tr>`;
    }).join('')}
  </tbody></table></div></div>
  <div class="panel" style="margin-top:22px;">
    ...
    ${(state.balanceLogs||[]).length===0?`<tr><td colspan="5"><div class="empty-state"><div class="em">🧾</div>Belum ada pengisian saldo.</div></td></tr>`:
...
function openBalanceForm(id){ state.formModal={type:'balance', sellerId:id}; render(); }
function submitBalanceForm(e){
  ...
  if(!seller) return;
  const amount = Math.round(Number(document.getElementById('bf-amount').value));
  const note = document.getElementById('bf-note').value.trim();
  if(!amount){ toast('Isi jumlah saldo (tidak boleh 0)'); return; }
  if(amount < 0 && sellerBalance(sellerId) + amount < 0){ toast('Pengurangan melebihi saldo mitra ('+fmtRp(Math.max(sellerBalance(sellerId),0))+')'); return; }
  state.balanceLogs.unshift({id:'b'+Date.now(), sellerId, amount, note, date:new Date().toISOString().slice(0,10), by:staff.name});
  persist(); state.formModal=null; render();
  toast('Saldo '+seller.name+(amount>0?' ditambah ':' dikurangi ')+fmtRp(Math.abs(amount)));
}
...
function pageStaffKeuangan(){
  ...
  let grossSales = 0, modalTerjual = 0, komisiTerbayar = 0;
  ...
  const allSellers = state.users.filter(u=>u.role==='member');
  const totalSaldoDiisi = allSellers.reduce((a,u)=>a+sellerCredited(u.id),0);
  const komisiPendingMitra = allSellers.reduce((a,u)=>a+sellerPending(u),0);
  const sisaSaldoMitra = totalSaldoDiisi - totalDitarik;
  ...
  <div class="stat-grid">
    <div class="stat-card"><div class="lbl">Total Dicairkan ke Mitra</div><div class="val">${fmtRp(totalDitarik)}</div></div>
    <div class="stat-card"><div class="lbl">Penarikan Menunggu</div><div class="val">${fmtRp(totalMenunggu)}</div></div>
    <div class="stat-card"><div class="lbl">Komisi Pending Mitra</div><div class="val">${fmtRp(komisiPendingMitra)}</div><div class="sub">Belum diisi ke saldo</div></div>
    <div class="stat-card"><div class="lbl">Saldo Mitra Belum Ditarik</div><div class="val">${fmtRp(Math.max(sisaSaldoMitra,0))}</div><div class="sub">Total diisi admin ${fmtRp(totalSaldoDiisi)}</div></div>
  ...
  <div class="panel">
    <div class="panel-head"><h3>Riwayat Penarikan Dana Mitra</h3></div>
    <div class="table-scroll"><table><thead><tr><th>Mitra</th><th>Jumlah</th><th>Metode</th><th>Nama Pemilik</th><th>Alamat Penarikan</th><th>Tanggal</th><th>Status</th></tr></thead><tbody>
    ...
    state.withdrawals.map(w=>{
      ...
      return `<tr><td>${s?s.name:'-'}</td><td>${fmtRp(w.amount)}</td><td>${w.method}</td><td>${w.holder||'-'}</td><td>${w.address||'-'}</td><td>${w.date}</td><td>${w.status==='Disetujui'?'<span class="badge badge-green">Disetujui</span>':w.status==='Ditolak'?'<span class="badge badge-red">Ditolak</span>':'<span class="badge badge-gold">Menunggu</span>'}</td></tr>`;
    }).join('')}
    </tbody></table></div>
  </div>`;
}
...
function PortalFormModals(){
  ...
  if(t==='proofview'){
    ...
    return `<div class="overlay" onclick="if(event.target===this) closeFormModal()"><div class="modal">
      ...
      <div style="padding:12px 14px;background:var(--card);border-radius:var(--radius-md);margin-bottom:14px;font-size:13.5px;line-height:1.7;">
        ...
        ${o.shipping?`<div><span style="color:var(--text-dimmer);">Alamat:</span> ${esc(o.shipping.recipient)} · ${esc(o.shipping.phone)} — ${esc(fmtAddress(o.shipping))}</div>`:''}
  ...
  if(t==='balance'){
    ...
    if(!m) return '';
    return `<div class="overlay" onclick="if(event.target===this) closeFormModal()"><div class="modal">
      <div class="modal-top"><h3>Isi Saldo Mitra</h3><button class="x-btn" onclick="closeFormModal()">✕</button></div>
      <form onsubmit="submitBalanceForm(event)">
        <div class="fake-stat"><div class="l">Mitra</div><div class="v">${esc(m.name)}</div></div>
        <div class="fake-stat"><div class="l">Komisi pending</div><div class="v">${fmtRp(sellerPending(m))}</div></div>
        <div class="fake-stat"><div class="l">Saldo saat ini</div><div class="v">${fmtRp(Math.max(sellerBalance(m.id),0))}</div></div>
        <div class="field" style="margin-top:14px;"><label>Jumlah (Rp)</label><input id="bf-amount" type="number" step="1" required placeholder="Contoh: 150000"><div class="field-hint">Isi angka negatif (mis. -50000) untuk mengoreksi/mengurangi saldo.</div></div>
        <div class="field"><label>Catatan (opsional)</label><input id="bf-note" placeholder="Contoh: Komisi order #1024, 1025"></div>
        <button class="btn btn-primary btn-block" type="submit">Simpan Saldo</button>
      </form></div></div>`;
  }
  if(t==='withdraw'){
