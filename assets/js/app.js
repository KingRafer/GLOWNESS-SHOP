...
===================================================== */
function buildWaMessage({name, wa, items, subtotal, total, refCode, orderId, paymentName}){
  let lines = [];
...
function pageAkunProfil(u){
  ...
  <div class="panel">
    ...
    <div class="field"><label>Alamat</label><textarea id="prof-address" placeholder="Alamat lengkap (jalan, kota, kode pos)">${esc(u.address||'')}</textarea></div>
...
function renderAkunCustomer(){
  ...
  return `<div class="dash-wrap">${CustomerSidebar(u, state.route)}<div class="dash-main">${content}</div></div>${PaymentInfoModal()}${Toast()}`;
}
...
function pageStaffPengguna(){
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Info</th><th>Alamat</th><th></th></tr></thead><tbody>
    ${state.users.map(u=>`<tr>
      ...
      <td style="max-width:220px;"><span style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${esc(u.address||'')}">${u.address?esc(u.address):'<span style="color:var(--text-dimmer);">Belum diisi</span>'}</span></td>
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Mitra</th><th>Kode Referral</th><th>Upline</th><th>Jaringan</th><th>Order</th><th>Komisi Pending</th><th>Saldo</th><th></th></tr></thead><tbody>
    ${sellers.map(s=>{
      ...
      return `<tr><td><b>${s.name}</b></td>
        ...
        <td><b>${fmtRp(bal)}</b></td>
        <td class="row-actions"><button class="btn btn-soft btn-sm" onclick="openBalanceForm('${s.id}')" title="Isi saldo mitra">+ Isi Saldo</button><button class="icon-btn" onclick="openPendingForm('${s.id}')" title="Ubah komisi pending">✎ Pending</button><button class="icon-btn" onclick="openUserForm('${s.id}')" title="Edit lengkap">✎</button></td></tr>`;
    }).join('')}
  </tbody></table></div></div>
