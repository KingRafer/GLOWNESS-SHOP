...
let state = {
  ...
  dashMenuOpen:false,
  aiOpen:false, aiMessages:[{role:'bot', text:'Halo! 👋 Saya asisten GLOWNESS SHOP. Ada yang bisa saya bantu soal produk, pembayaran, peluang bisnis, atau pesananmu?'}],
};
...
function Why(){
  ...
  <div class="wrap section">
    ...
    <div class="why-row">
      ...
      <div class="why-card"><div class="em">💬</div><h4>Siap Membantu</h4><p>Ada pertanyaan? Tim & asisten AI kami siap membantu kapan saja kamu butuh.</p></div>
    </div>
  </div>`;
}
...
function pageStaffSeller(){
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Mitra</th><th>Kode Referral</th><th>Upline</th><th>Jaringan</th><th>Order</th><th>Total Komisi</th><th></th></tr></thead><tbody>
    ${sellers.map(s=>{
...
===================================================== */
const AI_QUICK_REPLIES = ['Rekomendasi produk','Cara jadi mitra','Lacak pesanan saya','Cara pembayaran','Ongkos kirim'];
function AiWidget(){
...
function aiReplyFallback(raw){
  ...
  if(/seller|jual|komisi|reseller|afiliasi|referral/.test(t)){
    return `Belanja di GLOWNESS SHOP bisa mengubah pengeluaran jadi pemasukan! Setiap akun otomatis jadi mitra referral — dapatkan komisi dan bagikan kode referralmu untuk membangun jaringan matrix. Klik menu "Peluang Bisnis" untuk mendaftar.`;
  }
  ...
  if(/ongkir|kirim|pengiriman|sampai|estimasi/.test(t)){
    return `Pengiriman dilayani ke seluruh Indonesia. Konfirmasi ongkos kirim dilakukan lewat WhatsApp setelah pesanan dibuat.`;
  }
  if(/halo|hai|hi|pagi|siang|sore|malam/.test(t)){
    return `Hai juga! 👋 Ada yang bisa saya bantu soal produk, pembayaran, peluang bisnis, atau pesanan?`;
  }
