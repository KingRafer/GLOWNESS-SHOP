...
const SEED_USERS = [
  ...
  {id:'u3', name:'Bagas Wirawan', email:'seller@oliv.id', password:'seller123', role:'member', refCode:'BAGAS10', totalSpend:0, avatar:'', readNotifIds:[], parentId:null, sponsorId:null},
  {id:'u5', name:'Rina Anggraini', email:'rina@oliv.id', password:'seller123', role:'member', refCode:'RINA21', totalSpend:0, avatar:'', readNotifIds:[], parentId:'u3', sponsorId:'u3'},
  {id:'u6', name:'Dewi Kartika', email:'dewi@oliv.id', password:'seller123', role:'member', refCode:'DEWI37', totalSpend:0, avatar:'', readNotifIds:[], parentId:'u3', sponsorId:'u3'},
  {id:'u4', name:'Putri Lestari', email:'customer@oliv.id', password:'customer123', role:'member', refCode:'PUTRI44', totalSpend:620000, avatar:'', readNotifIds:[], parentId:null, sponsorId:null},
];
const SEED_ORDERS = [
  {id:'o1001', buyerName:'Fajar S.', buyerWa:'081234500011', total:220000, status:'Selesai', date:'2026-08-18', refCode:'BAGAS10', items:[{id:'p1',name:'Produk Glow A',qty:2,unitPrice:110000,basePrice:60000}], discount:0},
  {id:'o1002', buyerName:'Putri Lestari', buyerWa:'081234500022', total:135000, status:'Selesai', date:'2026-08-25', refCode:null, items:[{id:'p2',name:'Produk Glow B',qty:1,unitPrice:135000,basePrice:85000}], discount:0},
  {id:'o1003', buyerName:'Yoga R.', buyerWa:'081234500033', total:85000, status:'Diproses', date:'2026-09-01', refCode:'BAGAS10', items:[{id:'p3',name:'Produk Glow C',qty:1,unitPrice:85000,basePrice:45000}], discount:0},
  {id:'o1004', buyerName:'Nadia P.', buyerWa:'081234500044', total:270000, status:'Selesai', date:'2026-09-04', refCode:'BAGAS10', items:[{id:'p2',name:'Produk Glow B',qty:1,unitPrice:150000,basePrice:85000},{id:'p3',name:'Produk Glow C',qty:1,unitPrice:120000,basePrice:45000}], discount:0},
  {id:'o1005', buyerName:'Rian Hidayat', buyerWa:'081234500055', total:190000, status:'Diproses', date:'2026-09-06', refCode:null, items:[{id:'p1',name:'Produk Glow A',qty:2,unitPrice:95000,basePrice:60000}], discount:0},
];
...
const SEED_NOTIFICATIONS = [
  {id:'n1', title:'Selamat Datang Seller GLOWNESS SHOP! 🎉', message:'Terima kasih sudah bergabung. Yuk mulai bagikan link referralmu dan pantau komisimu di dashboard.', date:'2026-08-15', from:'Tim GLOWNESS SHOP'},
];
const SEED_ANNOUNCEMENT = {active:true, title:'Belanja Jadi Untung!', message:'Belanja di GLOWNESS SHOP, ubah pengeluaranmu jadi pemasukan. Daftar seller gratis dan mulai bangun jaringanmu hari ini.', date:'2026-09-01'};
...
let state = {
  ...
  dashMenuOpen:false,
  aiOpen:false, aiMessages:[{role:'bot', text:'Halo! 👋 Saya asisten GLOWNESS SHOP. Ada yang bisa saya bantu soal produk, pembayaran, peluang bisnis seller, atau pesananmu?'}],
};
...
async function initData(){
  ...
  state.users.forEach(u=>{ if(u.role==='member'){ if(u.parentId===undefined) u.parentId=null; if(u.sponsorId===undefined) u.sponsorId=null; delete u.packageId; delete u.resellPrices; } });
...
function resolveRefCode(raw){
  ...
  const seller = state.users.find(x=>x.role==='member' && x.refCode===code);
...
===================================================== */
function buildWaMessage({name, wa, items, subtotal, discountPct, total, refCode, orderId, paymentName}){
  let lines = [];
...
function Hero(){
  ...
  <div class="hero">
    <div class="wrap hero-grid">
      <div>
        <div class="kicker">Belanja Cerdas · Untung Bareng</div>
        <h1>Belanja di ${SITE_NAME}, <span class="gold">ubah pengeluaran jadi pemasukan.</span></h1>
        <p>Setiap belanja tak lagi sekadar mengeluarkan uang. Bersama ${SITE_NAME}, kamu menikmati produk pilihan sekaligus membuka peluang bisnis yang memberikan keuntungan untuk semua seller.</p>
        <div class="hero-actions">
          <button class="btn btn-primary" onclick="location.hash='#/produk'">Belanja Sekarang</button>
          <button class="btn btn-ghost" onclick="openAuth('register')">Mulai Bisnis Gratis</button>
        </div>
      </div>
      <div class="hero-side">
        <div class="row"><div class="ic">💸</div><div><h4>Belanja yang Menghasilkan</h4><p>Dari sekadar konsumen, jadilah seller dan dapatkan komisi dari setiap penjualan.</p></div></div>
        <div class="row"><div class="ic">🤝</div><div><h4>Untung untuk Semua Seller</h4><p>Bangun jaringan lewat kode referral dan tumbuh bersama dalam struktur matrix.</p></div></div>
...
function Why(){
  ...
  <div class="wrap section">
    <div class="section-head">
      ...
      <h2>Belanja yang membawa penghasilan</h2>
      <p>Kami menghadirkan cara belanja baru: kebutuhanmu terpenuhi, peluang bisnismu terbuka.</p>
    </div>
    <div class="why-row">
      <div class="why-card"><div class="em">💸</div><h4>Pengeluaran Jadi Pemasukan</h4><p>Belanja sekaligus jadi seller, lalu ubah kebiasaan belanjamu menjadi sumber penghasilan.</p></div>
      <div class="why-card"><div class="em">🤝</div><h4>Untung untuk Semua</h4><p>Sistem referral matrix membuat setiap seller ikut tumbuh bersama jaringannya.</p></div>
      <div class="why-card"><div class="em">🚀</div><h4>Mulai Tanpa Ribet</h4><p>Daftar gratis, bagikan kode referral, dan pantau komisi real-time.</p></div>
    </div>
  </div>`;
}
...
function ProdukSection(){
  ...
  <div class="wrap section" id="produk">
    <div class="section-head">
      ...
      <p>Belanja produk favoritmu, lalu jadikan setiap pembelian langkah awal bisnismu.</p>
    </div>
    ${ProductGrid()}
  </div>`;
}
function DiskonTeaser(){
  ...
  <div class="section-alt">
    <div class="wrap section">
      ...
      <div style="margin-top:22px;"><button class="btn btn-primary" onclick="openAuth('register')">Daftar Jadi Member</button></div>
    </div>
  </div>`;
}
function SellerCta(){
    ...
    <div class="reseller-cta">
      <div>
        ...
        <div class="reseller-points">
          ...
          <div class="pt"><div class="n">2</div><div><b>Bagikan kode referral</b><span>Ajak teman dan keluarga bergabung, dan bangun jaringan seller dalam struktur matrix.</span></div></div>
        ...
        <button class="btn btn-primary" onclick="openAuth('register')">Daftar Jadi Seller</button>
      </div>
      <div class="reseller-visual">
        <div class="fake-stat"><div class="l">Biaya pendaftaran seller</div><div class="v" style="color:var(--gold-2)">Gratis</div></div>
        <div class="fake-stat"><div class="l">Harga jual</div><div class="v">Atur sendiri</div></div>
...
function CheckoutModal(){
  ...
  const u = currentUser();
  const tier = (u && u.role==='member') ? tierFor(u.totalSpend||0) : null;
  ...
  <div class="overlay" onclick="if(event.target===this) closeFormModal()">
    <div class="modal">
      ...
      <form onsubmit="submitCheckout(event)">
        ...
        <div class="field"><label>Kode Referral / Diskon (opsional)</label><input id="co-ref" value="${esc(pendingRef())}" oninput="updateCheckoutSummary()" placeholder="Contoh: BAGAS10 atau WELCOME10">
...
function checkoutCalc(refInput){
  ...
  if(res.type==='seller'){
    ...
    let memberPct = (u && u.role==='member') ? tierFor(u.totalSpend||0).discount : 0;
...
function submitCheckout(e){
  ...
  const u = currentUser();
  const {order, c} = buildOrderObject(name, wa, refInput, pm ? pm.name : 'Konfirmasi WhatsApp');
  ...
  state.formModal = {type:'payinfo', orderId:order.id};
  render();
}
function PaymentInfoModal(){
  ...
  <div class="overlay" onclick="if(event.target===this) closeFormModal()">
    <div class="modal">
      <div class="modal-top"><h3>Pesanan #${o.id} Dibuat 🎉</h3><button class="x-btn" onclick="closeFormModal()">✕</button></div>
...
function confirmPaymentWa(id){
  ...
  openWhatsAppOrder({
    ...
    subtotal:o.items.reduce((a,i)=>a+i.unitPrice*i.qty,0), discountPct:o.discount||0, total:o.total,
    refCode:o.refCode||o.discountCode||null, paymentName:o.paymentMethod,
  });
}
  ...
  if(a.tab==='login'){
    ...
    <div class="overlay" onclick="if(event.target===this) closeAuth()">
      <div class="modal">
        ...
        <p style="font-size:11.5px;color:var(--text-dimmer);margin-top:16px;">Demo staf: owner@oliv.id / owner123 · admin@oliv.id / admin123 · Demo pengguna: seller@oliv.id / seller123 · customer@oliv.id / customer123</p>
      </div>
    </div>`;
  }
  // register: satu langkah — tidak ada pilihan peran, semua akun otomatis punya fitur customer + seller
  return `
  <div class="overlay" onclick="if(event.target===this) closeAuth()">
    <div class="modal">
...
function finishRegistration(){
  ...
  while(guard<20 && (state.users.some(x=>x.refCode===code) || (state.settings.referralCodes||[]).some(c=>c.code===code)));
...
function CustomerSidebar(u, active, unreadNotif){
  ...
  const items = [
    ...
    {icon:'📈', label:'Dashboard Seller', route:'#/seller'},
...
function pageSellerReferral(){
  ...
  <div class="dash-wrap">
    ...
    <div class="dash-main">
      <div class="pg-head"><div><h1>Referral & Matrix</h1><p>Bagikan kodemu, ajak seller baru, dan pantau pertumbuhan jaringan matrix-mu.</p></div></div>
      <div class="ref-box">
        <div>
      ...
      <div class="panel">
        ...
        <div class="field-hint" style="margin-bottom:14px;">Tiap seller punya maksimal ${width} posisi langsung, sedalam ${depth} level. Jika posisi sponsor penuh, seller baru otomatis ditempatkan di bawah jaringan sponsor (spillover) sehingga semua posisi terisi merata.</div>
      ...
      <div class="panel">
        ...
        <ul class="mx-tree mx-root"><li>
          ...
          <ul class="mx-tree">
            ...
            ${Array.from({length:emptySlots}).map(()=>`<li><div class="mx-node empty"><div class="av">＋</div><div><b>Slot kosong</b><small>Ajak seller baru dengan kodemu</small></div></div></li>`).join('')}
          </ul>
        </li></ul>
      </div>
    </div>
...
function ordersTableStaff(orders){
  if(orders.length===0) return `<div class="empty-state"><div class="em">🧾</div>Belum ada pesanan.</div>`;
  return `<div class="table-scroll"><table><thead><tr><th>ID</th><th>Pembeli</th><th>Produk</th><th>Total</th><th>Pembayaran</th><th>Referral</th><th>Status</th><th></th></tr></thead><tbody>
    ${orders.map(o=>`<tr>
...
function pageStaffPengguna(){
  return `
  <div class="pg-head"><div><h1>Pengguna</h1><p>Kelola akun staf dan pengguna (member dengan fitur customer & seller jadi satu).</p></div>
...
function pageStaffSeller(){
  const sellers = state.users.filter(u=>u.role==='member');
  return `
  <div class="pg-head"><div><h1>Seller</h1><p>Overview performa referral tiap pengguna. Kode referral bisa diubah langsung di sini.</p></div></div>
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Seller</th><th>Kode Referral</th><th>Upline</th><th>Jaringan</th><th>Order</th><th>Total Komisi</th><th></th></tr></thead><tbody>
    ${sellers.map(s=>{
      ...
      return `<tr><td><b>${s.name}</b></td>
        ...
        <td>${(state.users.find(x=>x.id===s.parentId)||{}).name||'—'}</td><td>${downlineCount(s.id)}</td><td>${os.length}</td><td>${fmtRp(comm)}</td>
...
function pageStaffKeuangan(){
  ...
  <div class="panel">
    ...
    <div class="table-scroll"><table><thead><tr><th>Seller</th><th>Jumlah</th><th>Metode</th><th>Nama Pemilik</th><th>Alamat Penarikan</th><th>Tanggal</th><th>Status</th></tr></thead><tbody>
...
function pageStaffPenarikan(){
  ...
  <div class="panel"><div class="table-scroll"><table><thead><tr><th>Seller</th><th>Jumlah</th><th>Metode</th><th>Nama Pemilik</th><th>Alamat Penarikan</th><th>Tanggal</th><th>Status</th><th></th></tr></thead><tbody>
...
function pageStaffPengaturan(){
  ...
  <div class="panel">
    <div class="panel-head"><h3>Struktur Matrix Referral Seller</h3></div>
    <div class="field-hint" style="margin-bottom:14px;">Lebar = jumlah posisi langsung per seller. Kedalaman = jumlah level yang dihitung & ditampilkan. Seller baru yang posisi sponsornya penuh otomatis ditempatkan di bawah jaringan sponsor (spillover).</div>
    <div class="reseller-price-row"><div class="nm">Lebar (posisi per seller)</div><input type="number" min="1" max="10" id="mx-width" value="${matrixCfg().width}"></div>
...
function submitReferralCodeForm(e){
  ...
  const dupSeller = state.users.find(u=>u.role==='member' && u.refCode===code);
    ...
    return `<div class="overlay" onclick="if(event.target===this) closeFormModal()"><div class="modal">
      ...
      <form onsubmit="submitUserForm(event)">
        ...
        <div class="field"><label>Role</label><select id="uf-role" onchange="document.getElementById('uf-ref-wrap').style.display=this.value==='member'?'block':'none'">
          ${['owner','admin','member'].map(r=>`<option value="${r}" ${u&&u.role===r?'selected':''}>${r==='member'?'member (customer + seller)':r}</option>`).join('')}
        </select></div>
...
function AiWidget(){
  ...
  <div class="ai-panel">
    <div class="ai-panel-head">
      <div><b>Asisten GLOWNESS SHOP</b><span class="ai-status"><span class="ai-status-dot"></span>Online</span></div>
...
function aiReplyFallback(raw){
  ...
  if(/seller|jual|komisi|reseller|afiliasi|referral/.test(t)){
    return `Belanja di GLOWNESS SHOP bisa mengubah pengeluaran jadi pemasukan! Setiap akun otomatis punya fitur customer & seller sekaligus — dapatkan komisi dan bagikan kode referralmu untuk membangun jaringan matrix. Klik menu "Peluang Bisnis" untuk mendaftar.`;
  }
  ...
  if(/bayar|pembayaran|transfer|metode|cara beli|checkout/.test(t)){
    return `Pilih metode di checkout: QRIS, GoPay, DANA, OVO, ShopeePay, atau transfer bank. Setelah pesanan dibuat, kamu akan melihat nomor/QR pembayaran, lalu kirim bukti bayar lewat WhatsApp untuk konfirmasi.`;
  }
  ...
  if(/halo|hai|hi|pagi|siang|sore|malam/.test(t)){
    return `Hai juga! 👋 Ada yang bisa saya bantu soal produk, pembayaran, seller, atau pesanan?`;
  }
